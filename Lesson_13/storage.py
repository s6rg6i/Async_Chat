...
if __name__ == '__main__':
    def fill_test_data(session_: Session):  # заполнение пустой базы тестовыми данными
        ...
        session_.add_all([
            User(login=f.user_name(), password=sha1(b'password').hexdigest(), public_key='') for _ in range(10)])
        session_.add_all([
            Room(name=f'Room_{num:02}') for num in range(1, 4)])
        session_.add_all([
        ...
        session_.add_all([RoomUser(room_id=1, user_id=i) for i in sample(range(2, 11), 7)])

        session_.add(RoomUser(room_id=2, user_id=2, permission=Permission.READ | Permission.WRITE | Permission.MODER))
        session_.add_all([RoomUser(room_id=2, user_id=i) for i in sample(range(3, 11), 6)])

        session_.add(RoomUser(room_id=3, user_id=10, permission=Permission.READ | Permission.WRITE | Permission.MODER))
        session_.add_all([RoomUser(room_id=3, user_id=i) for i in sample(range(1, 10), 7)])

        session.add_all([Message(
            room_id=randint(1, 3), user_id=randint(1, 11), msg=f.text(max_nb_chars=30),
            time=f.date_time_this_year()) for i in range(1, 50)])
    ...
    # chat_db.add_contact('caroline_watson', password=sha1(b'password').hexdigest())
